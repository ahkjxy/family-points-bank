...
export function MobileNav({ activeTab, onChangeTab, isAdmin, onProfileClick }: MobileNavProps) {
  ...
  return (
    <nav className="fixed bottom-3 left-0 right-0 z-40 mx-auto w-full max-w-3xl px-4 lg:hidden">
      <div className="bg-white/90 backdrop-blur-2xl border border-white/70 shadow-[0_24px_70px_-32px_rgba(15,23,42,0.65)] rounded-3xl p-2 flex items-stretch gap-2">
        {tabs.map(tab => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-2xl transition-all ${isActive ? 'bg-gradient-to-br from-[#FF4D94] to-[#7C4DFF] text-white shadow-lg shadow-[#7C4DFF]/30' : 'text-gray-500 hover:text-[#7C4DFF] hover:bg-white/70'}`}
            >
