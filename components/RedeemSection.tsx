...
export function RedeemSection({ rewards, balance, onRedeem }: RedeemSectionProps) {
  ...
  return (
    <div className="space-y-6 pb-16 animate-in zoom-in-95 duration-500">
      <div className="rounded-[28px] via-white border border-white shadow-[0_18px_60px_-36px_rgba(255,77,148,0.35)] p-5 sm:p-6 flex flex-col gap-4 mobile-card">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-4">
        {filtered.map(reward => {
          ...
          return (
            <div 
              key={reward.id} 
              className={`group relative h-full rounded-2xl border bg-white p-3 sm:p-4 flex flex-col gap-3 shadow-sm transition-all ${canAfford ? 'hover:-translate-y-1 hover:shadow-[0_18px_50px_-30px_rgba(15,23,42,0.35)] hover:border-[#FF4D94]/30' : 'border-gray-100 via-white'} mobile-card`}
            >
