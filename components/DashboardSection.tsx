...
export function DashboardSection({
  ...
  const allTransactions = useMemo<(Transaction & { profileId: string; profileName: string })[]>(
    () =>
      profiles.flatMap((p) =>
        p.history.map((tx) => ({ ...tx, profileId: p.id, profileName: p.name }))
      ),
    [profiles]
  );
  ...
  const anomalies = useMemo<string[]>(() => {
    const now = Date.now();
    const oneDay = 24 * 60 * 60 * 1000;
    const lastDayTx = allTransactions.filter(
      (t: Transaction & { profileName?: string }) => now - t.timestamp <= oneDay
    );
    ...
    const lastDayRedeemList = lastDayTx.filter(
      (t: Transaction & { profileName?: string }) => t.type === "redeem"
    );
    const lastDayRedeem = lastDayRedeemList.length;
    const largePenalty = allTransactions.find(
      (t: Transaction & { profileName?: string }) => t.type === "penalty" && t.points <= -20
    );
    const recentNames = Array.from(
      new Set(lastDayTx.map((t: { profileName?: string }) => t.profileName || "未知"))
    ).join("、");

    const flags: string[] = [];
    if (lastDayAbs >= 60 || Math.abs(lastDayNet) >= 40) {
      flags.push(
        `24小时内积分波动异常（${recentNames || "成员"}）：净变动 ${lastDayNet}，绝对值 ${lastDayAbs}`
      );
    }
    if (lastDayRedeem >= 3) {
      const redeemNames = Array.from(
        new Set(
          lastDayRedeemList.map(
            (t: Transaction & { profileName?: string }) => t.profileName || "未知"
          )
        )
      ).join("、");
    ...
    if (largePenalty) {
      flags.push(
        `发现大额扣减 ${largePenalty.points}：${largePenalty.title}（${largePenalty.profileName || "成员"}）`
      );
    }
    return flags;
  }, [allTransactions]);
  ...
  const messageCenter = useMemo<MessageItem[]>(() => {
    ...
    anomalies
      .slice(0, 2)
      .forEach((msg: string) =>
        items.unshift({ title: "异常警告", desc: msg, time: "刚刚", tone: "rose" })
      );
    items.push({
      title: "申诉 / 反馈",
      desc: "成员可提交问题，管理员处理后反馈",
      time: "随时",
      tone: "slate",
    });
    return items;
  ...
  const displayMessages = useMemo<MessageItem[]>(
    () => (openNotice ? messageCenter : messageCenter.slice(0, 4)),
    [messageCenter, openNotice]
  );
            ...
            return (
              <g key={i} className="group cursor-pointer">
                <circle
                  ...
                  cy={y}
                  r={6}
                  className="fill-white dark:fill-gray-900 stroke-[3] stroke-[#FF4D94] transition-all group-hover:r-8"
                />
                <rect
                  x={x - 15}
                  y={y - 32}
                  ...
                  height={20}
                  rx={6}
                  className="fill-gray-900 dark:fill-white opacity-0 group-hover:opacity-100 transition-opacity"
                />
                <text
                  x={x}
                  y={y - 18}
                  textAnchor="middle"
                  className="text-[10px] font-bold fill-white dark:fill-gray-900 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity"
                >
                  {d.value}
                </text>
                <text
                  ...
                  y={height + 15}
                  textAnchor="middle"
                  className="text-[10px] font-bold fill-gray-400 dark:text-gray-500 tabular-nums"
                >
                  {d.label}
                </text>
              </g>
    ...
    <div className="space-y-6 lg:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="lg:col-span-8 bg-gradient-to-br from-[#1A1A1A] to-[#333] dark:from-[#0F172A] dark:to-[#1E293B] p-8 lg:p-10 rounded-[40px] text-white shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)] relative overflow-hidden flex flex-col min-h-[320px] border border-white/5">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.4em] text-[#FF4D94] mb-2">
                  当前余额
                </p>
                <div className="flex items-baseline gap-3">
                  <h3 className="text-5xl md:text-7xl font-black points-font tracking-tighter">
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FF4D94]/10 border border-[#FF4D94]/20 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#FF4D94] animate-pulse"></div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-[#FF4D94]">
                    当前会话活跃
                  </span>
                </div>
            ...
            <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div
                  key={i}
                  className="p-4 rounded-[24px] bg-white/5 border border-white/10 hover:bg-white/10 transition-colors group"
                >
                  <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1 group-hover:text-white/60">
                    {stat.label}
                  </p>
                  <p
                    className={`text-xl font-black points-font ${
                      stat.trend === "emerald"
                        ? "text-emerald-400"
                        : stat.trend === "rose"
                          ? "text-rose-400"
            ...
            <div className="mt-auto pt-10 flex flex-wrap gap-4">
              <button
                onClick={onGoEarn}
                className="px-8 py-4 bg-[#FF4D94] text-white rounded-[20px] text-sm font-black uppercase tracking-widest hover:scale-[1.05] active:scale-95 transition-all shadow-[0_20px_40px_-12px_rgba(255,77,148,0.5)] flex items-center gap-2"
              >
              ...
              <button
                onClick={onGoRedeem}
                className="px-8 py-4 bg-white/10 text-white border border-white/10 backdrop-blur-md rounded-[20px] text-sm font-black uppercase tracking-widest hover:bg-white/20 active:scale-95 transition-all flex items-center gap-2"
              >
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="flex-1 bg-white dark:bg-[#1E293B] p-6 rounded-[32px] border border-gray-100 dark:border-white/5 shadow-sm flex flex-col justify-center relative overflow-hidden group hover:border-[#FF4D94]/30 transition-colors">
            ...
            <div className="relative z-10">
              <p className="text-[10px] font-black text-[#FF4D94] uppercase tracking-[0.3em] mb-1">
                最近动态
              </p>
              {lastTx ? (
                <>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium italic">
                    {formatDateTime(lastTx.timestamp)}
                  </p>
                  <div
                    className={`mt-4 inline-flex items-center px-3 py-1 rounded-full text-xs font-black ${
                      lastTx.points > 0
              ...
              <div className="mt-auto pt-4 flex items-center gap-2">
                <div className="flex -space-x-2">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className="w-6 h-6 rounded-full border-2 border-[#7C4DFF] bg-white/20 backdrop-blur-sm"
                    ></div>
                  ))}
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-[#0F172A] p-8 rounded-[40px] border border-gray-100 dark:border-white/5 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <div>
              <p className="text-[11px] font-black text-[#FF4D94] uppercase tracking-[0.3em] mb-1">
                元气数据分析
              </p>
            ...
            <div className="flex gap-2 p-1 bg-gray-50 dark:bg-white/5 rounded-xl">
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${chartView === "week" ? "bg-white dark:bg-white/10 shadow-sm text-gray-900 dark:text-white" : "text-gray-400 hover:text-gray-600"}`}
              >
                周视图
              </button>
              <button
                onClick={() => setChartView("month")}
                className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${chartView === "month" ? "bg-white dark:bg-white/10 shadow-sm text-gray-900 dark:text-white" : "text-gray-400 hover:text-gray-600"}`}
              >
                月视图
              </button>
            </div>
          ...
          <div className="space-y-8">
            <div className="p-6 bg-gray-50/50 dark:bg-white/5 rounded-[32px] border border-gray-100 dark:border-transparent">
              {renderLineChart(
                chartView === "week" ? weekly : monthly,
                chartView === "week" ? maxWeek : maxMonth
              )}
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
              {[
                {
                  label: "单日峰值",
                  value: `+${chartView === "week" ? maxWeek : maxMonth}`,
                  icon: "plus",
                },
                {
                  label: "日均活跃",
                  value: (
                    (chartView === "week" ? weekly : monthly).reduce(
                      (s, d) => s + Math.abs(d.value),
                      0
                    ) / (chartView === "week" ? 7 : 30)
                  ).toFixed(1),
                  icon: "history",
        ...
        <div className="lg:col-span-4 bg-white dark:bg-[#0F172A] p-8 rounded-[40px] border border-gray-100 dark:border-white/5 shadow-sm">
          <p className="text-[11px] font-black text-[#7C4DFF] uppercase tracking-[0.3em] mb-1">
            荣誉榜
          </p>
          <h3 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight mb-8">
            成员积分榜
          </h3>
          <div className="space-y-4">
            {topMembers.map((m, idx) => (
              <div
                key={m.id}
                className="group flex items-center justify-between p-4 rounded-[28px] bg-gray-50/50 dark:bg-white/5 border border-transparent hover:border-[#7C4DFF]/30 transition-all"
              >
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black text-lg shadow-lg ${m.avatarColor}`}
                    >
                      {m.name[0]}
                    </div>
                ...
                <div
                  className={`text-right ${m.total >= 0 ? "text-emerald-500" : "text-rose-500"}`}
                >
                  <p className="text-lg font-black points-font">
                    {m.total >= 0 ? "+" : ""}
                    {m.total}
                  </p>
          ...
          <button className="w-full mt-6 py-4 bg-gray-50 dark:bg-white/5 rounded-2xl text-[11px] font-black text-gray-400 uppercase tracking-widest hover:bg-gray-100 transition-colors">
            查看所有成员
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#0F172A] p-8 lg:p-10 rounded-[40px] border border-gray-100 dark:border-white/5 shadow-sm overflow-hidden relative">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-8">
            <div>
              <p className="text-[11px] font-black text-emerald-500 uppercase tracking-[0.3em] mb-1">
                成长洞察
              </p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">
                成员画像与成长走势
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {profileInsights.map((p) => (
              <div
                key={p.id}
                className="bg-gray-50/30 dark:bg-white/5 p-6 rounded-[32px] border border-gray-100 dark:border-transparent hover:shadow-xl transition-all group"
              >
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg ${p.avatarColor} group-hover:rotate-12 transition-transform`}
                    >
                      {p.name[0]}
                    </div>
                    <div>
                  ...
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center text-[10px] font-black border-2 ${
                      p.completionRate >= 80
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    ...
                    <div className="w-24 h-12">{renderMiniTrend(p.trend7d)}</div>
                  </div>
                  <div className="w-full h-1.5 bg-gray-100 dark:bg-white/10 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-emerald-400 to-[#FF4D94] transition-all duration-1000"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Recent Bills */}
        <div className="bg-white dark:bg-[#1E293B] p-6 rounded-[32px] border border-gray-100 dark:border-white/5 shadow-sm flex flex-col relative overflow-hidden group hover:border-[#FF4D94]/30 transition-colors">
          <div className="absolute top-[-20px] right-[-20px] text-gray-50 dark:text-white/5 group-hover:text-[#FF4D94]/10 transition-colors">
          ...
          <div className="relative z-10 flex items-center justify-between mb-8">
            ...
            <button className="text-[10px] font-black text-[#FF4D94] uppercase tracking-widest hover:underline">
              查看全部
            </button>
          </div>
          <div className="space-y-4">
            {currentProfile.history.slice(0, 5).map((h) => (
              <div key={h.id} className="flex items-center justify-between group">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-gray-800 dark:text-gray-200 truncate max-w-[120px]">
                      {h.title}
                    </p>
                ...
                <span
                  className={`text-sm font-black points-font ${h.points > 0 ? "text-emerald-500" : "text-rose-500"}`}
                >
                  {h.points > 0 ? "+" : ""}
                  {h.points}
                </span>
          ...
          <div className="space-y-3">
            {anomalies.length === 0 ? (
              ...
              anomalies.map((msg, idx) => (
                <div
                  key={idx}
                  className="p-4 bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 rounded-[20px] text-xs font-bold text-rose-600 dark:text-rose-400 leading-relaxed flex gap-3"
                >
        ...
        <div className="bg-white dark:bg-[#1E293B] p-6 rounded-[32px] border border-gray-100 dark:border-white/5 shadow-sm flex flex-col relative overflow-hidden group hover:border-[#FF4D94]/30 transition-colors">
          ...
          <div className="relative z-10 flex items-center justify-between mb-8">
            ...
            <button
              ...
              className="text-[10px] font-black text-[#7C4DFF] uppercase tracking-widest hover:underline"
            >
              {openNotice ? "收起" : "展开"}
            </button>
          </div>
            ...
            {displayMessages.map((msg, idx) => (
              <div key={idx} className="flex gap-4 group">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-black text-gray-800 dark:text-gray-200 leading-tight group-hover:text-[#FF4D94] transition-colors">
                    {msg.title}
                  </p>
                  <p className="text-[12px] text-gray-500 dark:text-gray-400 mt-0.5 truncate">
                    {msg.desc}
                  </p>
                  <p className="text-[10px] font-black text-gray-400 dark:text-gray-600 uppercase mt-1 tracking-widest">
                    {msg.time}
                  </p>
                </div>
              </div>
